...
export function parseValue(raw: string): number {
  const n = parseFloat(raw.trim().replace(/^[¥$€£₩]/, '').replace(/,/g, '').replace(/%$/, ''));
...
function cells(row: Element): HTMLTableCellElement[] {
  return Array.from(row.children).filter((c): c is HTMLTableCellElement => c.tagName === 'TH' || c.tagName === 'TD');
}
...
export function parseTable(table: HTMLTableElement): ParsedData {
  ...
  if (thead) { hr = trs(thead)[0]; dr = rows; }
  else if (rows.length) { hr = rows[0]; dr = rows.slice(1); }
  ...
  for (const row of dr) {
    ...
    for (let i = 0; i < headers.length; i++) datasets[i]?.push(parseValue(cs[i + 1]?.textContent?.trim() ?? ''));
  }
