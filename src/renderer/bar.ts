...
export function renderBar(data: ParsedData, config: ChartConfig, colors: string[]): SVGSVGElement {
  ...
  const ng = data.labels.length,
    ns = data.headers.length;
    ...
    for (let i = 0; i < ng; i++) {
      ...
      if (config.stacked) {
        ...
        addBar(
          g,
          gc - bw / 2,
          layout.paddingTop + layout.chartHeight - (cum / maxValue) * layout.chartHeight - sh,
          bw,
          sh,
          s === ns - 1 ? config.radius : 0,
          c,
        );
      } else {
        const v = sr[i] ?? 0;
        ...
        addBar(
          ...
          gc - (ns * (bw + 2)) / 2 + s * (bw + 2),
          v >= 0
            ? layout.paddingTop + layout.chartHeight - bh
            : layout.paddingTop + layout.chartHeight,
          bw,
          bh,
          config.radius,
          c,
...
function renderH(data: ParsedData, config: ChartConfig, colors: string[]): SVGSVGElement {
  ...
  for (let i = 0; i < nr; i++) {
    const y = 20 + i * (bh + rg),
      ...
      w = (Math.abs(v) / mx) * cw;
    g.appendChild(
      svgEl("rect", { x: lm, y, width: cw, height: bh, rx: config.radius, fill: gc, opacity: 0.4 }),
    );
