...
export function getStackedMaxValue(ds: number[][]): number {
  ...
  for (let i = 0; i < n; i++) {
    let s = 0;
    for (const d of ds) s += Math.max(0, d[i] ?? 0);
    if (s > m) m = s;
  }
  return m || 1;
}
...
function renderChrome(
  ...
  const gc = getThemeColor("grid"),
    tc = getThemeColor("textMuted");
  // Grid
  if (cfg.grid !== "none") {
    const g = svgEl("g", { class: P + "grid" });
    if (cfg.grid === "y" || cfg.grid === "both")
      for (let i = 0; i <= 4; i++) {
        const y = l.paddingTop + l.chartHeight * (1 - i / 4);
        g.appendChild(
          svgEl("line", {
            x1: l.paddingLeft,
            y1: y,
            x2: l.width - l.paddingRight,
            y2: y,
            stroke: gc,
            "stroke-width": 1,
          }),
  ...
  svg.appendChild(yg);
  // X-axis
  const xg = svgEl("g", { class: P + "axis" }),
    gw = l.chartWidth / labels.length;
  for (let i = 0; i < labels.length; i++) {
    const t = svgEl("text", {
      x: l.paddingLeft + gw * i + gw / 2,
      y: l.paddingTop + l.chartHeight + 20,
      "text-anchor": "middle",
      "font-size": 12,
      "font-family": FF,
      fill: tc,
...
export function setupChart(
  ...
  data: ParsedData,
  mv?: number,
): { svg: SVGSVGElement; layout: ChartLayout; maxValue: number } {
...
export function renderLegend(
  ...
  if (
    cfg.legend === "none" ||
    (data.headers.length <= 1 && cfg.legend !== "top" && cfg.legend !== "bottom")
  )
    return;
...
export function computePoints(sr: number[], n: number, l: ChartLayout, max: number): Point[] {
  ...
  for (let i = 0; i < n; i++)
    p.push({
      x: n === 1 ? l.paddingLeft + l.chartWidth / 2 : l.paddingLeft + (i / (n - 1)) * l.chartWidth,
      y: l.paddingTop + l.chartHeight - ((sr[i] ?? 0) / max) * l.chartHeight,
    });
  return p;
}
...
export function renderCircularLegend(
  svg: SVGSVGElement,
  labels: string[],
  vals: number[],
  total: number,
...
export function addBar(
  ...
  g.appendChild(
    svgEl("rect", { class: P + "bar", x, y, width: w, height: h, rx, fill: c, opacity: 0.9 }),
  );
}
