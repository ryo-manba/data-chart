...
import { setupChart, seriesGroup, renderLegend, svgEl, computePoints, buildPath, renderDots, renderLinePath } from './common';
...
export function renderArea(data: ParsedData, config: ChartConfig, colors: string[]): SVGSVGElement {
  ...
  const n = data.labels.length, bg = getThemeColor('bg');
  ...
  for (let s = 0; s < data.headers.length; s++) {
    ...
    g.appendChild(svgEl('path', { class: 'data-chart-area', d: `${lp} L${lx},${bottom} L${fx},${bottom} Z`, fill: c, opacity: 0.15 }));
